...
string modRoutingCipher::encrypt(const string &open_text) { setlocale(LC_ALL, "Russian");
locale loc("ru_RU.UTF-8");
wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t> codec; 
wstring op = codec.from_bytes(open_text);
int LENGTH = op.size(); wchar_t ch[LENGTH+1];
int KEY = key-1;
int k = 0; int i = 0;
while(KEY>=0) {
k = KEY;
...
int colichestvoxv = key - colichestvonex; int KEY = LENGTH-1; 
int key1 = key; 
int i = 0, j = 0, k = 0, y = 0;
if(colichestvonex==0) {
key1 = 0;
